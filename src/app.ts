import { ChronicleDB, migrateLocal, openStore, prepareImage } from './db.js';
import { CloudService } from './cloud.js';
import {
  ABILITIES, AbilityKey, CharacterRecord, ChronicleRecord, PartyRecord, SectionRecord, SessionRecord,
  clampScore, escapeHtml as esc, modifier, sanitizeHtml, signed
} from './model.js';

const SECTION_KINDS: Record<'party'|'character', Array<[string,string]>> = {
...
class ChroniclesApp {
  ...
  private records: ChronicleRecord[] = [];
  private mediaUrls = new Map<string,string>();
  private sort: 'new'|'name' = 'new';
  private selectedCharacter: string | null = null;
  private readonly root = document.querySelector<HTMLElement>('#app')!;
  private readonly overlay = document.querySelector<HTMLElement>('#overlay')!;
  private readonly toastNode = document.querySelector<HTMLElement>('#toast')!;
  private readonly cloudStatus = document.querySelector<HTMLButtonElement>('#cloud-status')!;
  ...
  async boot(): Promise<void> {
    ...
    this.cloud = new CloudService(
      this.db,
      async next => { this.db = next; await this.refresh(); },
      message => { this.cloudStatus.textContent = message; },
      async () => this.refresh()
    );
  ...
  private async refresh(): Promise<void> {
    this.records = await this.db.all<ChronicleRecord>('records');
    this.releaseMedia();
    const route = decodeURIComponent(location.hash.slice(1) || 'parties');
    if (route === 'character') this.renderCharacter();
    else if (route.startsWith('party/')) this.renderParty(route.slice(6));
    else this.renderParties();
    await this.hydrateMedia();
    this.updateNav(route);
  }

  private active<T extends ChronicleRecord>(type: T['type'], parentId?: string): T[] {
    return this.records.filter(record => record.type === type && !record.deleted_at && (!parentId || record.parent_id === parentId)) as T[];
  }

  private byId<T extends ChronicleRecord>(id: string): T | undefined {
    return this.records.find(record => record.id === id) as T | undefined;
  }

  private updateNav(route: string): void {
  ...
  private heading(title: string, subtitle?: string): string {
  ...
  private renderParties(): void {
    const parties = this.active<PartyRecord>('party').sort((a,b) => this.sort === 'name'
  ...
  private partyCard(party: PartyRecord): string {
    const sessions = this.active<SessionRecord>('session', party.id).length;
  ...
  private sectionRecord(parent: ChronicleRecord, kind: string, title: string): SectionRecord {
  ...
  private sectionView(section: SectionRecord): string {
  ...
  private renderParty(id: string): void {
    const party = this.byId<PartyRecord>(id);
    if (!party || party.deleted_at) { location.hash = 'parties'; return; }
    const sections = SECTION_KINDS.party.map(([kind,label]) => this.sectionView(this.sectionRecord(party, kind, label))).join('');
    const sessions = this.active<SessionRecord>('session', party.id).sort((a,b) => String(b.date||'').localeCompare(String(a.date||'')));
  ...
  private sessionCard(session: SessionRecord): string {
    return `<article class="session-card"><div class="session-top"><div><span class="eyebrow">${this.formatDate(session.date||'')}${session.location?` · ${esc(session.location)}`:''}</span><h3>${esc(session.title)}</h3></div><button class="text-action" data-action="edit-session" data-id="${esc(session.id)}">✎ Сведения</button></div>${session.image_id?`<div class="session-image" data-media="${esc(session.image_id)}"></div>`:''}${session.description?`<p>${esc(session.description)}</p>`:''}${this.sectionView({...session, title:'Запись сессии'} as unknown as SectionRecord)}</article>`;
  }

  private renderCharacter(): void {
    const characters = this.active<CharacterRecord>('character');
  ...
  private abilityCard(character: CharacterRecord, key: AbilityKey, short: string, full: string, icon: string): string {
  ...
  private async hydrateMedia(): Promise<void> {
    for (const node of document.querySelectorAll<HTMLElement>('[data-media]')) {
      const id = node.dataset.media;
      if (!id) continue;
      const media = await this.db.get<any>('media', id);
      if (!media?.blob) continue;
      const url = URL.createObjectURL(media.blob);
      this.mediaUrls.set(id, url);
      const image = new Image(); image.src = url; image.alt = '';
  ...
  private releaseMedia(): void { for (const url of this.mediaUrls.values()) URL.revokeObjectURL(url); this.mediaUrls.clear(); }
  private formatDate(value: string): string { if (!value) return ''; const date = new Date(`${value}T12:00:00`); return Number.isNaN(date.getTime()) ? '' : new Intl.DateTimeFormat('ru-RU',{day:'numeric',month:'short',year:'numeric'}).format(date); }
  ...
  private async onClick(event: MouseEvent): Promise<void> {
    ...
    if (action === 'open-party') { location.hash = `party/${id}`; return; }
    if (action === 'new-party') return this.openRecordDialog('party');
    if (action === 'edit-party') return this.openRecordDialog('party', id);
    if (action === 'new-character') return this.openRecordDialog('character');
    if (action === 'edit-character') return this.openRecordDialog('character', id);
    if (action === 'select-character') { this.selectedCharacter = id; return this.refresh(); }
    if (action === 'new-session') return this.openRecordDialog('session', undefined, id);
    if (action === 'edit-session') return this.openRecordDialog('session', id);
    if (action === 'edit-section') return this.openEditor(id);
    if (action === 'close-overlay') return this.closeOverlay();
    if (action === 'cloud') return this.openCloudDialog();
    if (action === 'delete') return this.deleteRecord(id);
  }

  private async onChange(event: Event): Promise<void> {
    const target = event.target as HTMLInputElement | HTMLSelectElement;
    if (target.id === 'sort') { this.sort = target.value as 'new'|'name'; await this.refresh(); }
  }

  private showOverlay(content: string, extraClass=''): void {
  ...
  private closeOverlay(): void { this.overlay.className='overlay'; this.overlay.innerHTML=''; document.body.classList.remove('modal-open'); }

  private field(label: string, name: string, value = '', type = 'text', full = false): string {
  ...
  private openRecordDialog(type: 'party'|'character'|'session', id?: string, parentId?: string): void {
    const record = id ? this.byId<any>(id) : {} as any;
    const title = id ? 'Править сведения' : type==='party' ? 'Новая партия' : type==='character' ? 'Создать персонажа' : 'Новая сессия';
    let fields = '';
    if (type === 'party') fields = `${this.field('Название','title',record.title||'')}${this.field('Количество игроков','players_count',record.players_count||'0','number')}${this.field('Описание','description',record.description||'','textarea',true)}${this.field('Дата начала','start_date',record.start_date||'','date')}<label class="form-field"><span>Статус</span><select name="status">${['В процессе','На паузе','Завершена'].map(value=>`<option ${record.status===value?'selected':''}>${value}</option>`).join('')}</select></label>${this.field('Отряд','party',record.party||'','textarea',true)}${this.field('Текущая цель','goal',record.goal||'','textarea',true)}`;
    if (type === 'session') fields = `${this.field('Название сессии','title',record.title||'')}${this.field('Дата','date',record.date||'','date')}${this.field('Место','location',record.location||'')}${this.field('Описание','description',record.description||'','textarea',true)}`;
    if (type === 'character') fields = `${this.iconField('Имя','name',record.name||'','●','Введите имя персонажа…')}${this.iconField('Раса','race',record.race||'','❧','Укажите расу…')}${this.iconField('Класс','class',record.class||'','⚔','Укажите класс…')}${this.iconField('Предыстория','background',record.background||'','▤','Укажите предысторию…')}<div class="section-label full"><i></i><span>Характеристики</span><i></i></div><div class="ability-input-grid full">${ABILITIES.map(a=>this.abilityInput(a.key,a.short,a.icon,record[a.key]??10)).join('')}</div>`;
    ...
    document.querySelector<HTMLFormElement>('#record-form')!.addEventListener('submit', event => void this.saveRecord(event));
    const file = document.querySelector<HTMLInputElement>('#record-form input[type=file]');
    file?.addEventListener('change', () => { const name = document.querySelector<HTMLElement>('.upload-name'); if (name) name.textContent = file.files?.[0]?.name || 'JPG, PNG, WEBP или GIF · до 20 МБ'; });
    document.querySelectorAll<HTMLInputElement>('.ability-input input').forEach(input => input.addEventListener('input', () => { const badge = input.closest('.ability-input')!.querySelector('em')!; badge.textContent = signed(modifier(clampScore(input.value))); }));
  }

  private iconField(label:string,name:string,value:string,icon:string,placeholder:string): string {
  ...
  private abilityInput(key: AbilityKey, label: string, icon: string, value: number): string {
    const score = clampScore(value); return `<label class="ability-input"><span class="ability-icon">${icon}</span><b>${label}</b><input name="${key}" type="number" min="1" max="30" value="${score}"><em>${signed(modifier(score))}</em></label>`;
  }

  private uploadField(label:string, portrait:boolean): string {
  ...
  private async saveRecord(event: SubmitEvent): Promise<void> {
    event.preventDefault();
    const form = event.currentTarget as HTMLFormElement;
    const type = form.dataset.type as 'party'|'character'|'session';
    const id = form.dataset.id || crypto.randomUUID();
    const data = new FormData(form);
    const values: Record<string,unknown> = {};
    for (const [key,value] of data.entries()) if (key !== 'image') values[key] = typeof value === 'string' ? value.trim() : value;
    if (type==='party') values.players_count = Number(values.players_count)||0;
    if (type==='session' && form.dataset.parent) values.parent_id = form.dataset.parent;
    if (type==='character') for (const ability of ABILITIES) values[ability.key] = clampScore(values[ability.key]);
    ...
    this.closeOverlay();
    if (type==='party') location.hash=`party/${id}`;
    if (type==='character') { this.selectedCharacter=id; location.hash='character'; }
    await this.refresh();
    this.cloud.schedule();
  ...
  private async openEditor(id: string): Promise<void> {
    let record = this.byId<any>(id);
    if (!record) {
      const split = id.lastIndexOf(':');
      const parent = this.byId<any>(id.slice(0,split));
      const kind = id.slice(split+1);
      const label = parent ? SECTION_KINDS[parent.type as 'party'|'character']?.find(([k])=>k===kind)?.[1] : undefined;
    ...
    document.querySelectorAll<HTMLButtonElement>('[data-command]').forEach(button => button.addEventListener('mousedown', e => e.preventDefault()));
    document.querySelectorAll<HTMLButtonElement>('[data-command]').forEach(button => button.addEventListener('click', () => { editor.focus(); document.execCommand(button.dataset.command!, false); }));
    document.querySelector<HTMLButtonElement>('#save-editor')!.addEventListener('click', async () => {
      await this.db.change(record.type,id,{...record,content:sanitizeHtml(editor.innerHTML),legacy_plain:false},null,id);
      this.closeOverlay(); await this.refresh(); this.cloud.schedule(); this.toast('Сохранено');
    });
  }

  private async deleteRecord(id: string): Promise<void> {
    const record = this.byId(id); if (!record) return;
    if (!confirm('Удалить запись? Действие можно отменить только из резервной копии.')) return;
    await this.db.change(record.type,id,{deleted_at:new Date().toISOString()});
    if (record.type==='party') location.hash='parties';
    await this.refresh(); this.cloud.schedule();
  }
  ...
  private openCloudDialog(): void {
    ...
    document.querySelector<HTMLButtonElement>('#sync-now')?.addEventListener('click',()=>void this.cloud.sync());
    document.querySelector<HTMLButtonElement>('#enable-cloud')?.addEventListener('click',async()=>{await this.cloud.useAccount(true); this.closeOverlay();});
    document.querySelector<HTMLButtonElement>('#logout')?.addEventListener('click',async()=>{await this.cloud.logout(); this.closeOverlay();});
    document.querySelector<HTMLFormElement>('#auth-form')?.addEventListener('submit', async event => {
      event.preventDefault(); const form=event.currentTarget as HTMLFormElement; const data=new FormData(form); const mode=(event as SubmitEvent).submitter as HTMLButtonElement;
      try { const ok=await this.cloud.login(String(data.get('email')),String(data.get('password')),mode.value==='register'); if(ok) this.openCloudDialog(); else form.querySelector('.form-error')!.textContent='Подтвердите email по ссылке в письме.'; }
      catch(error){ form.querySelector('.form-error')!.textContent=error instanceof Error?error.message:'Ошибка входа'; }
    });
  }

  private toast(message:string):void { this.toastNode.textContent=message; this.toastNode.hidden=false; window.setTimeout(()=>this.toastNode.hidden=true,2600); }
}

new ChroniclesApp().boot().catch(error => {
  const root=document.querySelector('#app'); if(root) root.innerHTML=`<main class="fatal"><h1>Журнал не открылся</h1><p>${esc(error instanceof Error?error.message:String(error))}</p></main>`;
});
