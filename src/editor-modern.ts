...
document.addEventListener('focusin', event => {
  ...
  if (target instanceof HTMLElement && target.matches('#editor[contenteditable="true"]')) historyFor(target);
});

document.addEventListener('input', event => {
  const target = event.target;
  if (target instanceof HTMLElement && target.matches('#editor[contenteditable="true"]')) remember(target);
}, true);
